...
def display_dashboard(start_date, end_date, target):
    tab1, tab2 = st.tabs(["Poppulation change", "Compare"])
    
    with tab1:
        st.subheader(f"Population change form {start_date} to {end_date}")
...
if __name__ == "__main__":
    ...
    st.markdown("Source table can be found here")
    ...
    with st.form("population-form"):
        ...
        with col1:
            ...
            start_year = st.slider("Year", min_value=1991, max_value=2023, value=1991, key="start_y")
    ...
    if start_date not in df['Quarter'].tolist() or end_date not in df['Quarter'].tolist():
        st.error("No data available. Check your quarter and your selection")  
